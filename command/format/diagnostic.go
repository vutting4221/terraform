...
func Diagnostic(diag tfdiags.Diagnostic, sources map[string][]byte, color *colorstring.Colorize, width int) string {
	...
	//
	// We currently use "ASCIIArt-like" characters for this, which is not great
	// because it creates ambiguity for non-visual terminals. Once we're no
	// longer restricted by the limitations of Windows Console in versions
	// earlier than 10 (which don't not support Unicode) it would be nice to
	// replace this with true line-drawing characters: ╷, │, and ╵ for
	// start, line, and end respectively.
	...
	if desc.Detail != "" {
		paraWidth := width - leftRuleWidth // leave room for the left rule
		if paraWidth > 0 {
